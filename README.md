# Ulysses Protocol — Solana

A behavioral commitment protocol for Web3 traders. Stake tokens to pre-commit to NOT buying a target token. Violators get slashed; compliant stakers earn yield.

> "Turn your discipline into yield. Turn others' FOMO into your reward."
...
| **Program ID** | `7s1UK1nQWK7CrNcaS576gbpMjMqrph1vArRepnQYLki7` |
| **Treasury Pubkey** | `mkzyaL8Xie6T3GHGrfCydeMk6JEajEWnkxrmjwZNgLJ` |

Explorer: [View on Solana Explorer](https://explorer.solana.com/address/7s1UK1nQWK7CrNcaS576gbpMjMqrph1vArRepnQYLki7?cluster=devnet)

## How It Works

1. **Stake** — user stakes TOKEN_A and commits to not buying TOKEN_B for duration T
2. **Monitor** — watcher service tracks TOKEN_B SPL balance via Helius webhooks
3. **Slash** — violation detected → `slash()` redistributes stake to compliant stakers via O(1) accumulator
4. **Claim** — no violation + duration expired → `claim()` returns principal + accumulated yield
